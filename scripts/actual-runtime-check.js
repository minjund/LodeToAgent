...
(async () => {
  ...
  try {
    ...
    const metrics = await evaluate(send, `(() => ({
      ...
      legacyProtectionCopy: /지시 내용은 보호된 세션|로컬 로그에서 보호|지시 원문은 Codex 로컬 로그에서 암호화|실제로 보낸 작업 지시는|도움 AI에게 일을 맡기기 직전/.test(document.body.innerText),
