...
const releaseDir = path.resolve(process.env.LOADTOAGENT_RELEASE_DIR || path.join(__dirname, '..', 'release'));
const artifacts = fs.readdirSync(releaseDir)
  .filter(name => /^LoadToAgent(?:-Setup)?-[0-9].*\.exe$/i.test(name) || /^LoadToAgent-[0-9].*-portable\.exe$/i.test(name))
...
for (const artifact of artifacts) {
  ...
  const subject = execFileSync(powershell, ['-NoProfile', '-NonInteractive', '-Command', script], {
    env: { ...process.env, LOADTOAGENT_ARTIFACT: artifact },
    encoding: 'utf8',
    windowsHide: true,
  }).trim();
