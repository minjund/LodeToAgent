...
Whitebox stores managed-run metadata and terminal replay locally so sessions
...
an active task is not destroyed. Set `WHITEBOX_RETENTION_DAYS` to a whole
...
This policy covers Whitebox's managed-run and terminal persistence only.
Provider-owned Claude, Codex, Gemini, or Grok logs follow each provider's policy
and are never deleted by Whitebox.
